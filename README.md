...
- 角色创建器（/make）：参照 LingChat 0.4.1 的创建流程，四步向导，**完成后打包下载、不直接发布**
  ...
  3. 高级设置：立绘参数（缩放/偏移/气泡/思考语/宠物）、系统提示词、服装列表（名称+提示词+当前服装）、语音设置（引擎/语言 + 各引擎专属字段按需显隐）
  4. 导出打包：生成 LingChat 角色目录格式的 zip（settings.yml + ai模式<角色名>.txt + avatar/ 立绘，哭泣→伤心、难为情→羞耻），解压到 LingChat 的 data/game_data/characters/ 即可使用
